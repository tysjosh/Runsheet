...
class AuthUserRow:
    ...
    idempotency key (Req 9.4); ``roles`` carries only the canonical role names
    (``admin`` / ``dispatcher`` / ``ops_manager`` / ``driver``); ``st_user_id``
    is the SuperTokens user id once backfilled.
    """
