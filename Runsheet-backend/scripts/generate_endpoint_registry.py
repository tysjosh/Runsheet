...
def generate_registry() -> str:
    ...
    # Deliberately no generation timestamp in the output.
    #
    # The header used to carry `on {YYYY-MM-DD HH:MM UTC}`, which made the
    # artifact irreproducible: regenerating it changed one line even when not a
    # single route had moved. The CI drift guard is a bare
    # `git diff --exit-code docs/endpoint-registry.md`, so that check could not
    # pass on any day after the file was last committed — it reported the
    # registry as stale, with the remedy already applied. It went unnoticed
    # because the job `needs: backend-tests`, which was failing earlier for an
    # unrelated reason, so this one never ran.
    #
    # Git already records when the file changed and who changed it, so the
    # timestamp carried no information the repository did not already hold. What
    # it cost was the ability to tell a real route change from the clock moving.
    lines: List[str] = [
        "# Endpoint Registry",
        "",
