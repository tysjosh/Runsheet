"""
Order Creation Service — shared "stamp + persist + emit + broadcast" sequence.
...
Step 6 used to dual-write the new order into the legacy
``shipments_current`` surface through ``LegacyDualWriter``. That shim
has been retired along with the legacy surface it fed.

Constructor deps:
    order_repo, ws_manager, optional feature_flag_service, optional clock.
...
class OrderCreationService:
    ...
    sequence for new order creation.
    ...
    Constructor dependencies:
        ...
        feature_flag_service: Optional ``FeatureFlagService`` for overlay
            state checks. Retained for callers even though the legacy
            dual-write it gated has been removed.
        clock: Optional clock override for testing (defaults to
            ``services.time_utils.utcnow``).

    NB: the ``legacy_dual_writer`` dependency was dropped with the
    legacy mirror — new orders are no longer projected into
    ``shipments_current``.
    """
    ...
    async def create_order(
        ...
        """Create a new order: stamp, persist, emit event, broadcast.
        ...
        # NB: step 6 used to dual-write the order into the legacy
        # shipment surface via LegacyDualWriter. The mirror has been
        # retired, so creation now ends at the broadcast.
    ...
    # ------------------------------------------------------------------
    # ``_dual_write_legacy_if_enabled`` lived here. It read the
    # ``order_intake_pipeline`` overlay state and, in ``shadow`` /
    # ``active_gated``, mirrored the new order into ``shipments_current``
    # through LegacyDualWriter. Both the shim and the legacy surface are
    # gone, so the step was deleted rather than left permanently inert.
